...
// Delivers the Supabase Auth invite link generated by
// app/api/branch-employees/invite/route.ts. Email, not WhatsApp: a
// business-initiated WhatsApp message to someone who's never messaged
// Umuhle needs a pre-approved Meta template (see lib/whatsapp.ts's
// sendPhoneOtp for the one that already exists, "umuhle_number_otp") —
// there's no invite-link template approved yet, and getting one approved
// is an external, multi-day Meta review process outside this codebase.
// Email has no equivalent constraint, so that's the channel for now.
export async function sendEmployeeInviteEmail(opts: {
  ...
  await sendToAll([ADMIN_EMAIL, opts.toEmail], {
    subject:     `You've been added to ${opts.storeName} on Umuhle`,
    template:    "employee_invite",
    referenceId: opts.toEmail,
    text:        `Hi ${opts.employeeName},\n\n${opts.inviterName} has added you as a team member at ${opts.branchName} (${opts.storeName}) on Umuhle.\n\nSet your password to activate your account:\n${opts.activationLink}\n\nThe Umuhle Team`,
    html:        emailWrapper("You're on the team", `
      <p style="margin:0 0 1rem">Hi ${opts.employeeName},</p>
      <p style="margin:0 0 1.5rem;font-size:0.85rem;color:#666">${opts.inviterName} has added you as a team member at <strong>${opts.branchName}</strong> (${opts.storeName}) on Umuhle. Set your password to activate your account and see your bookings.</p>
      <p style="margin:0 0 1.5rem"><a href="${opts.activationLink}" style="display:inline-block;background:#9B7FB8;color:#fff;font-weight:600;text-decoration:none;padding:0.75rem 1.5rem;border-radius:10px">Activate my account</a></p>
      <p style="margin:0 0 1.5rem">The Umuhle Team</p>`),
  });
}
