...
export function buildPaymentParams(options: {
  ...
  email: string;
  baseUrl: string;
  customStr1?: string; // payment_type: "booking"|"order"|"store_booking_deposit"
  customStr2?: string; // extra reference
  /**
   * Instantly routes part of this payment to a partner's own PayFast
   * account — see docs.payfast.co.za's Split Payments section. Only ever
   * set this when the recipient's merchant ID has actually been added to
   * Umuhle's "Allowed merchants" list in the PayFast dashboard first (see
   * profiles.payfast_split_approved) — an un-allow-listed merchant_id
   * here fails the whole transaction, not just the split.
   *
   * ⚠️ VERIFY BEFORE GOING LIVE: the `setup`/`split_payment` JSON shape
   * below (merchant_id + amount, both snake_case) is PayFast's documented
   * structure as of this writing, but this codebase has never sent it to
   * PayFast for real. Run one real sandbox transaction with a test
   * secondary merchant ID and confirm the split actually lands before
   * relying on this in production — see lib/payments/eligibility.ts's
   * splitPaymentEligibility() for where this gets wired in.
   */
  split?: { merchantId: string; amountCents: number };
  ...
  // `setup` is deliberately added AFTER signing — PayFast's own docs say
  // it's excluded from the signature calculation (confirmed against a
  // PayFast support thread: "The split payment parameters needs to be
  // sent in the setup and it is not calculated in the signature").
  // Including it before signing would make PayFast reject every split
  // payment with a signature mismatch.
  if (options.split) {
    params.setup = JSON.stringify({
      split_payment: {
        merchant_id: options.split.merchantId,
        amount: options.split.amountCents,
      },
    });
  }
