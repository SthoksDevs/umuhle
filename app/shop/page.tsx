...
export default function ShopPage() {
  ...
  const handleAdd = (product: Product) => {
    // Guest checkout means adding to cart never needs an account — the
    // cart lives in localStorage and carries straight through to checkout
    // (see app/checkout/page.tsx). Only wishlist saving (below) still
    // requires signing in, since there's no guest concept for a saved list.
    addItem(product, 1);
    setAdded(product.id);
  ...
  return (
    <>
      ...
      <div style={{ minHeight: "100vh", background: "var(--white)", fontFamily: "var(--font-body)", display: "flex", flexDirection: "column" }}>
        ...
        {/* Auth modal — wishlist saving is the only shop-page action left
            that requires an account (guests can add to cart and check out
            freely; see handleAdd above). */}
        {showAuth && (
