      ...
      <div style={{ background: "#fff", borderRadius: 24, padding: "3rem 2.5rem", maxWidth: 480, width: "100%", textAlign: "center", boxShadow: "0 24px 80px rgba(155,127,184,0.15)" }}>
        ...
        {ref && (
          <p style={{ fontSize: "0.8rem", color: "var(--light)", marginBottom: "1.5rem" }}>
            Order reference: <span style={{ fontFamily: "monospace" }}>{ref}</span>
          </p>
        )}
        <p style={{ color: "var(--grey)", fontSize: "0.9rem", marginBottom: "2rem" }}>
          You will receive a WhatsApp confirmation shortly with your order details and tracking information.
        </p>

        <div style={{ display: "flex", gap: "0.75rem", justifyContent: "center", flexWrap: "wrap" }}>
          <Link href="/dashboard">
            <button className="btn-plum">View my orders</button>
          </Link>
          <Link href="/shop">
...
export default function PaymentSuccessPage() {
  return (
    <Suspense fallback={<div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}><Image src={ICON} alt="Umuhle" width={48} height={48} style={{ borderRadius: "50%" }} /></div>}>
      <SuccessContent />
    </Suspense>
  );
}
